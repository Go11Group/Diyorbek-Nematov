...
func (l *LessonRepo) CreateLesson(lesson models.Lesson) error {
	...
	`, lesson.CourseID, lesson.Title, lesson.Content, time.Now(), time.Now())
...
func (l *LessonRepo) UpdateLesson(lesson models.Lesson) error {
	...
	query += "updated_at = CURRENT_TIMESTAMP WHERE lesson_id = :id AND deleted_at = 0"
	params["id"] = lesson.ID
...
func (l *LessonRepo) DeleteLesson(id string) error {
	res, err := l.DB.Exec(`
		...
		WHERE lesson_id = $1 AND deleted_at = 0
	`, id)
