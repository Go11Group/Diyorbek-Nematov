...
func (u *UserRepo) GetUserByID(id string) (models.User, error) {
	...
	`).Scan(&user.ID, &user.Name, &user.Email, &user.Birthday, &user.Password)
	return user, err
}
...
func (u *UserRepo) GetAllUsers(fUser models.FilterUser) ([]models.User, error) {
	...
	query := "SELECT id, name, email, birthday, password FROM users WHERE deleted_at = 0 "
	...
	if fUser.Birthday != nil {
		params["birthday"] = fUser.Birthday.Format("2006-01-02")
	}
	if fUser.Offset > 0 {
	...
	for rows.Next() {
		...
		err = rows.Scan(&user.ID, &user.Name, &user.Email, &user.Birthday)
